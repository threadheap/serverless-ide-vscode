...
test("Resolving $refs", function(testDone) {
  let service = new SchemaService.JSONSchemaService(
    requestServiceMock,
    workspaceContext
  );
  ...
  service
    ...
    .then(solvedSchema => {
      expect(solvedSchema.schema.properties["child"]).toEqual({
        id: "https://myschemastore/child",
        type: "bool",
        description: "Test description"
      });
...
test("Resolving $refs 2", function(testDone) {
  ...
  service
    ...
    .then(fs => {
      expect(fs.schema.properties["responseValue"]).toEqual({
        type: "object",
        required: ["$ref"],
...
test("Resolving $refs 3", function(testDone) {
  ...
  service
    ...
    .then(fs => {
      expect(fs.schema.properties["p1"]).toEqual({
        type: "string",
        enum: ["object"]
      });

      expect(fs.schema.properties["p2"]).toEqual({
        type: "string",
        enum: ["object"]
      });
      expect(fs.schema.properties["p3"]).toEqual({
        type: "string",
        enum: ["object"]
      });
    })
...
test("FileSchema", function(testDone) {
  ...
  service
    ...
    .then(fs => {
      ...
      expect(section.description).toBe("Meaning of Life");
    })
    .then(
      () => testDone(),
      error => {
...
test("Schema not found", function(testDone) {
  ...
  service
    ...
    .then(schema => {
      expect(schema.errors).toHaveLength(1);
    })
    .then(
      () => testDone(),
      error => {
